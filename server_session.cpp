...
Session::ReceiveResult ServerSession::receiveMessage(BYTE* buffer, size_t bufferSize, unsigned int timeout_sec, BYTE** ppPayload, size_t* pPayloadSize, ServerSession** ppChildSession, unsigned int* pChildSessionId)
{
    ...
    if (header->sessionId == 0) // new session
    {
        ...
        memcpy_s(newSession->_remoteDhPublicKeyBuffer, DH_KEY_SIZE_BYTES, buffer + sizeof(MessageHeader), header->payloadSize);

        if (!CryptoWrapper::startDh(&newSession->_dhContext, newSession->_localDhPublicKeyBuffer, DH_KEY_SIZE_BYTES)) {
            printf("Error in starting Diffie Hellman while creating a new server session\n");
            return RR_FATAL_ERROR;
        }
        ...
        if (!CryptoWrapper::getDhSharedSecret(newSession->_dhContext, newSession->_remoteDhPublicKeyBuffer, DH_KEY_SIZE_BYTES, newSession->_sharedDhSecretBuffer, DH_KEY_SIZE_BYTES)) {
            printf("Error in getting the shared secret while creating a new server session\n");
            return RR_FATAL_ERROR;
        }
